...
posixTest("materialization rejects an Organization-local url.insteadOf rewrite before target claim", async () => {
  const root = await createLaunchpadGitFixture();
  tempRoots.push(root);
  const organizationRoot = join(root, "organizations", "BetaCo_GEN3");
  const target = join(organizationRoot, "workspace", "private-module");
  const declaredRemote = "git://127.0.0.1:1/declared-private-module.git";
  const foreignRemote = join(root, "remotes", "foreign-module.git");
  await prepareOrganizationRoot(organizationRoot);
  await mkdir(join(root, "sources"), { recursive: true });
  await initGitRepo(join(root, "sources", "foreign-module"), { remotePath: foreignRemote });
  await appendFile(
    join(organizationRoot, ".git", "config"),
    `\n[url "${foreignRemote}"]\n\tinsteadOf = ${declaredRemote}\n`,
  );
  const repo = await createManifestRepo({
    root,
    organizationRoot,
