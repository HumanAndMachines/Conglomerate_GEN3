...
| 0130 | Principál určuje záměr a drží publikační rozhodnutí, ale nemusí být architekt; Worker Agent nese odbornou odpovědnost za Lazurio-konformní návrh každé source změny. Povinný `architecture-shaping` skill odděluje záměr od mechanismu, preferuje jednu autoritu, nativní best practices, konvergenci a důkaz, škáluje podle rizika a není závislý na konkrétním reviewerovi nebo modelu. V source stromu aktualizovaného Lazurio rootu platí tato baseline i při vstupu do nested Organization, modulu a Personalspace; self-contained template a live-repo kopie ji přebírají řízeným rolloutem evidovaným v root issues. |
