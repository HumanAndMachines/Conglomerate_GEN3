...
- Zástupce míří na stabilní bootstrap pod
  `%LOCALAPPDATA%\HumanAndMachine\Launchpad`; pracovní složku zástupce drží
  stejný LocalAppData adresář, nikoli Git checkout.
- Bootstrap čte `install.json` s jediným kanonickým rootem, ověří
  `launchpad.gen3.json` a odmítne přímou `.worktrees` cestu i existující
  junction/symlink alias. Je to trusted-local integrity guard proti přetrvalé
  omylem uložené cestě, ne TOCTOU sandbox proti souběžnému zásahu
  stejného uživatele,
  který už může měnit config i checkout.
...
- Reinstalace kontroluje jen doloženou legacy task identitu v root TaskPath `\`:
  `HumanAndMachine Launchpad GEN3`; pouze pokud její akce míří do `.worktrees`,
  bezpečně ji vypne. Úlohu nemaže a reportuje přesnou identitu, akci a výsledek
  pro rollback/audit.
- Ikona, bootstrap i config se nejdřív dokončí v unikátním staging souboru
  stejného adresáře a pak se atomicky publikují; config jde až jako poslední.
  Existující cíl se nahrazuje přímo, first-install move při závodu failne
  zavřeně místo přepsání souběžně vytvořeného souboru.
...
- Regresní test hlídá bootstrap, kanonický marker, junction alias, exact
  scheduled-task quarantine a prefixovou kolizi.
...
3. Ve Windows CI runneru vytvořit task v root TaskPath `\` se **stejnou exact
   legacy identitou** `HumanAndMachine Launchpad GEN3` a akcí pod `.worktrees`,
   znovu spustit instalátor a ověřit stav `Disabled` bez smazání úlohy;
   prefixová kolize i stejný název mimo root TaskPath musí zůstat aktivní.
